...
export default class RectContainer extends BaseContainer implements IContainerNode {
    nodeInBound(ele: BaseNode): boolean {
        let r2 = ele.rectCase();
        if (r2[0] >= 0 && r2[1] >= 0 && r2[2] <= this.width && r2[3] <= this.height) {
            return true;
        } else {
            return false;
        }
    ...
    constructor(text: string, x: number, y: number, width: number = 200, height: number = 200) {
        super();
        this.text = text;
        this.x = x;
        this.y = y;
    ...
    paint(ctx: CanvasRenderingContext2D, ps: { focusEle: BaseElement | null }): void {
        ...
        if (this.text) {
            ctx.save();
            ctx.fillStyle = this.selected ? this.fillStyle_text_focus : this.fillStyle_text;
